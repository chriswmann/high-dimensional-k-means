...
run_clustering <- function(ndim, k, sd_mult, verbose) {
  ...
  for (row in 1:nrow(perms)) {
    ...
    if (pc_complete > 0 & pc_complete %% 1 == 0 & verbose) {
      print(paste(Sys.time(), ": ", pc_complete, "% complete."))
    }
  }
...
params <- data.frame(ndim = c(2, 4, 8, 16, 32, 64, 128, 256),
                     sd_mult = c(rep(1, 8), rep(2, 8), rep(3, 8), rep(4, 8)))
n_clusters = 4
...
## Analysis with Ten Clusters
Next we try the same analysis, this time with 10 clusters.  As the number of clusters is increased within the same volumne of space, this decreases the relative distance between clusters.  We therefore expect the clustering performance (accuracy) to decrease.
```{r run_analysis_with_ten_clusters}
start_time <- proc.time()
n_clusters = 10
results <- numeric(nrow(params))
for (i in 1:nrow(params)) {
  output <- run_clustering(ndim = params[i, 1], k = n_clusters, sd_mult = params[i, 2], verbose = TRUE)
  results[i] <- output$best_acc
}
results <- cbind(params, results)
end_time <- proc.time()
...
## Results with Ten Clusters
```{r plot_ten_cluster_results}
results %>% 
  ggplot(aes(x = as.factor(ndim), y = results, colour = as.factor(sd_mult))) +
  geom_line(aes(group = as.factor(sd_mult))) +
  geom_point() +
